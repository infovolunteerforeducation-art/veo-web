...
import CampPaymentClient from "@/components/trai-he/CampPaymentClient";
...
export default async function CampThanhToanPage({
  params,
}: {
      ...
      <main className="min-h-screen bg-surface py-8 px-4">
        <div className="max-w-2xl mx-auto">
          ...
          <nav className="flex items-center gap-1.5 text-xs text-on-surface-variant mb-6 flex-wrap">
            ...
            <span>/</span>
            <Link href={`/trai-he-tinh-nguyen/${camp.slug}/dang-ky`} className="hover:text-primary transition-colors">Đăng ký</Link>
            <span>/</span>
          ...
          <CampPaymentClient
            ...
            title={camp.title}
            price={camp.price}
            priceNumber={camp.priceNumber}
            dateRange={camp.dates[0]?.label ?? ""}
          />
        </div>
      </main>
      <Footer />
